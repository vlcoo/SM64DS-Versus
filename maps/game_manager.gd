...
func _ready() -> void:
	...
	spawn_player()
	Utils.set_discord_status(Utils.RPC_STATUS.InGame, map_name, $GameHUD.time_remaining)


func spawn_player(controlled: bool = true) -> Player:
	...
	current_player.camera_follow = controlled
...
func _on_timer_match_timeout() -> void:
	$GameResults.appear()
	$"3D".process_mode = Node.PROCESS_MODE_DISABLED
